...
public class BikeopdeiaService {
	...
	@HystrixCommand(fallbackMethod = "BikeopediaDetailsFallback", threadPoolKey = "BikopediaThreadPool", threadPoolProperties = {
	...
	public BikeOutput getBikeopediaDetails(BikeInput input) {
		...
		// db log
		UWLogging logdata = logging
				.save(new UWLogging("getBikeopediaDetails", input.toString(), null, null, new Date()));
		// try {
		// rest call
		...
		ResponseEntity<BikeOutput> entity = restTemplate.exchange("http://bike-o-pedia/mca/bikes", HttpMethod.POST,
				httpentity, BikeOutput.class);
	...
	public BikeOutput BikeopediaDetailsFallback(BikeInput input) {
		...
		// db log
		UWLogging logdata = logging
				.save(new UWLogging("BikeopediaDetailsFallback", input.toString(), null, null, new Date()));
